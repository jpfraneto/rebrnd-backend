...
async function migrateData() {
  ...
  if (!newConfig.database || !newConfig.username || !newConfig.password) {
    console.error('❌ ERROR: Missing required new database environment variables');
    process.exit(1);
  }
  ...
  try {
    ...
    newDataSource = new DataSource({
      ...
      entities: [User, Brand, Category, Tag, BrandTags, UserBrandVotes, UserDailyActions],
    ...
    const [prodCategories] = await prodConnection.execute<mysql.RowDataPacket[]>(
    ...
    if (prodBrands.length > 0) {
      ...
      const values = prodBrands.map(brand => {
        processed++;
        ...
        const validCategoryId = categoryId && categoryMap.has(categoryId) ? categoryId : null;
        
        // Transform ranking from INT to STRING
        const ranking = brand.ranking !== null && brand.ranking !== undefined
        ...
        return `(
          ...
          ${escapeValue(ranking)},
          ${escapeValue(brand.score || 0)},
          ${escapeValue(brand.stateScore || brand.state_score || 0)},
          ${escapeValue(brand.scoreWeek || brand.score_week || 0)},
          ${escapeValue(brand.stateScoreWeek || brand.state_score_week || 0)},
          ${escapeValue(brand.rankingWeek || brand.ranking_week || 0)},
          ${escapeValue(brand.scoreMonth || brand.score_month || 0)},
          ${escapeValue(brand.stateScoreMonth || brand.state_score_month || 0)},
          ${escapeValue(brand.rankingMonth || brand.ranking_month || 0)},
          ${escapeValue(brand.bonusPoints || brand.bonus_points || 0)},
      ...
      try {
        console.log(`  Inserting all ${prodBrands.length} brands in one batch...`);
        const queryRunner = newDataSource.createQueryRunner();
        await queryRunner.query(insertSQL);
        await queryRunner.release();
        
        console.log(`  ✓ Migrated all ${processed} brands in single batch using raw SQL`);
      } catch (error: any) {
      ...
      console.log(`  ✓ Migrated ${stats.brands} brands successfully using raw SQL`);
    } else {
    ...
    if (prodUsers.length > 0) {
      ...
      for (let i = 0; i < prodUsers.length; i += batchSize) {
        ...
        try {
          ...
          console.log(`  ✓ Migrated batch ${Math.ceil((i + batchSize) / batchSize)}/${Math.ceil(prodUsers.length / batchSize)} (${processed}/${prodUsers.length} users)`);
        } catch (error: any) {
          console.error(`  ❌ Failed to migrate user batch starting at ${i + 1}: ${error.message}`);
          console.error(`  SQL: ${insertSQL.substring(0, 500)}...`);
          throw error;
        }
      }
      ...
      console.log(`  ✓ Migrated ${stats.users} users successfully using raw SQL`);
    } else {
    ...
    if (prodVotes.length > 0) {
      ...
      console.log(`  Loaded ${allUsers.length} users and ${allBrands.length} brands`);
      ...
      for (let i = 0; i < prodVotes.length; i += batchSize) {
        ...
        for (const vote of batch) {
          ...
          if (user) {
            ...
            const validBrand1 = brand1Id && brandMap.has(brand1Id) ? brand1Id : null;
            const validBrand2 = brand2Id && brandMap.has(brand2Id) ? brand2Id : null;
            const validBrand3 = brand3Id && brandMap.has(brand3Id) ? brand3Id : null;
        ...
        if (validVotes.length > 0) {
          ...
          // Build VALUES clause for batch insert
          const values = validVotes.map(vote => {
            return `(
              ${escapeValue(vote.id)},
          ...
          const insertSQL = `
            INSERT INTO user_brand_votes (
              id, userId, brand1Id, brand2Id, brand3Id, date, shared, castHash,
              transactionHash, rewardAmount, day, shareVerified, shareVerifiedAt,
          ...
          try {
            ...
            console.log(`  ✓ Migrated batch ${Math.ceil((i + batchSize) / batchSize)}/${Math.ceil(prodVotes.length / batchSize)} (${processed}/${prodVotes.length} votes, ${skipped} skipped)`);
          } catch (error: any) {
            console.error(`  ❌ Failed to migrate votes batch starting at ${i + 1}: ${error.message}`);
            console.error(`  SQL: ${insertSQL.substring(0, 500)}...`);
            throw error;
          }
        } else {
          console.log(`  ✓ Skipped batch ${Math.ceil((i + batchSize) / batchSize)} (no valid votes)`);
        }
      }
      
      stats.userBrandVotes = prodVotes.length - skipped;
      console.log(`  ✓ Migrated ${stats.userBrandVotes} user brand votes successfully (${skipped} skipped due to missing users)`);
    } else {
      console.log('  ✓ No user brand votes to migrate');
    }

    // ========================================================================
    // Step 7: Migrate UserDailyActions
    // ========================================================================
    console.log('\nStep 7: Migrating UserDailyActions...');
      ...
      for (const action of prodActions) {
        ...
        if (user) {
          const newAction = actionsRepo.create({
            id: action.id,
            user: user,
            shareFirstTime: action.shareFirstTime || action.share_first_time || false,
          });
    ...
    console.log('');
    console.log('Note: New entities (AirdropScore, AirdropSnapshot, AirdropLeaf, RewardClaim)');
