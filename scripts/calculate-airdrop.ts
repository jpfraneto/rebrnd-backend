...
//   console.log('# ==============================================================================');
//   console.log('');
//   console.log('This will calculate airdrop scores for the top 1111 users based on:');
//   console.log('  • Base points from voting and activity');
//   console.log('  • Multipliers (follow accounts, channel interaction, BRND holdings, etc.)');
...
//       console.error('Set APP_URL environment variable if using a different URL');
//       process.exit(1);
//     }

//     console.log('');
//     console.log('📊 Starting airdrop calculation for top 1111 users...');
//     console.log('  This may take a few minutes depending on the number of users');
//     console.log('');

//     // Calculate airdrop scores for all top users via HTTP endpoint
//     const batchSize = 50;
//     console.log(`  Calling: GET ${baseUrl}/airdrop-service/calculate-all-users?batchSize=${batchSize}`);

//     const calculateResponse = await fetch(`${baseUrl}/airdrop-service/calculate-all-users?batchSize=${batchSize}`, {
//       method: 'GET',
//       headers: {
...
//       throw new Error(`Airdrop calculation failed: ${calculateResponse.status} ${calculateResponse.statusText}`);
//     }
...
//     console.log(`  • Total token allocation: ${result.totalTokens?.toLocaleString() || 'N/A'}`);
//     console.log('');

//     // Get final leaderboard to verify
//     console.log('🏆 Fetching final leaderboard (top 10)...');
//     console.log(`  Calling: GET ${baseUrl}/airdrop-service/leaderboard?limit=10`);

//     const leaderboardResponse = await fetch(`${baseUrl}/airdrop-service/leaderboard?limit=10`, {
//       method: 'GET',
//       headers: {
//         'Content-Type': 'application/json',
//       },
...
//           const percentage = leaderboard[0].finalScore > 0
//             ? ((user.finalScore / leaderboard[0].finalScore) * 100).toFixed(1)
//             : '0.0';
//           console.log(`    ${index + 1}. FID ${user.fid} - Score: ${user.finalScore.toLocaleString()} (${percentage}%)`);
//         });
//       } else {
...
//     console.log('# ==============================================================================');
//     console.log('');
//     console.log('The top 1111 users now have calculated airdrop scores and token allocations.');
//     console.log('The daily cron job will keep these scores updated going forward.');
//     console.log('');
